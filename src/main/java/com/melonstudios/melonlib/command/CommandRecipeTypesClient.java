...
public class CommandRecipeTypesClient extends CommandBase {
    @Override
    public String getName() {
        return "recipe_types_client";
    }

    @Override
    public String getUsage(ICommandSender sender) {
        return "command.recipe_types_client.usage";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if ("list".equals(args[0])) {
            ...
            for (String name : names) {
                if (RecipeRegistry.getRecipeTypeClient(name) == null) continue;
                out.append(name).append(" (").append(RecipeRegistry.getRecipeTypeClient(name).getAllRecipeIDs().size()).append(")").append("\n");
            }
        } else if ("get".equals(args[0])) {
            if (args.length == 1) throw new WrongUsageException("/recipe_types get <id>");
            String id = args[1];
            Collection<String> names = RecipeRegistry.getRecipeTypeClient(id).getAllRecipeIDs();
            if (!names.isEmpty()) {
            ...
            for (String type : types) {
                if (RecipeRegistry.getRecipeTypeClient(type) == null) continue;
